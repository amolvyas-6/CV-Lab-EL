...
import type { BackendJobRecord, ExtractionPayload, SummaryRow } from './types.ts'
...
export default function App() {
	...
	const [metadataContent, setMetadataContent] = useState<Record<string, unknown> | null>(null)
	...
	const refreshMetadata = useCallback(async () => {
		...
		try {
			...
			setMetadataError(
				error instanceof Error ? error.message : 'Unable to load metadata.json.',
			)
		}
	}, [])
	...
	const refreshSummary = useCallback(async () => {
		...
		try {
			...
			setSummaryError(
				error instanceof Error ? error.message : 'Unable to load dataset_summary.csv.',
			)
		}
	}, [])
	...
	const checkHealth = useCallback(async () => {
		...
		try {
			...
			setHealthState('ok')
			setHealthMessage(payload.status === 'ok' ? 'Backend online' : payload.status)
		} catch (error) {
			setHealthState('error')
			setHealthMessage(error instanceof Error ? error.message : 'Backend unavailable')
		}
	}, [])
	...
	const buildPayload = (): ExtractionPayload => {
		...
		if (minLon >= maxLon || minLat >= maxLat) {
			throw new Error('Bounding box must satisfy min < max for both longitude and latitude.')
		}
		...
		return {
			...
			cloud_filter_pct: parseNumber(form.cloudFilterPct, 'Cloud filter percent'),
	...
	const handleExtractSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
		...
		try {
			...
			setExtractMessage(error instanceof Error ? error.message : 'Failed to submit extraction.')
		}
	}
	...
	return (
		<div className="min-h-screen p-4 md:p-8 max-w-7xl mx-auto space-y-8 animate-in fade-in duration-700">
				...
				<div className="relative z-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
					<div className="space-y-4 max-w-3xl">
						...
						<h1 className="text-4xl md:text-5xl font-extrabold text-slate-900 leading-tight">
							Ship Dataset <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-600 to-blue-600">Extraction</span>
						</h1>
						<p className="text-lg text-slate-600 font-medium leading-relaxed">
							Launch high-fidelity extraction jobs, monitor real-time processing phases, and inspect generated metadata from a centralized operational dashboard.
						</p>
					</div>
					...
					<div className="flex items-center gap-3">
						...
						>
							<RefreshCw className={`w-5 h-5 ${healthState === 'loading' ? 'animate-spin text-emerald-600' : ''}`} />
						</button>
						<div className={`px-4 py-2.5 rounded-xl border flex items-center gap-2 shadow-sm ${
							healthState === 'ok' ? 'bg-emerald-50 border-emerald-100 text-emerald-700' : 
							healthState === 'error' ? 'bg-rose-50 border-rose-100 text-rose-700' : 
							'bg-slate-50 border-slate-200 text-slate-600'
						}`}>
							<div className={`w-2 h-2 rounded-full ${
								healthState === 'ok' ? 'bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)] animate-pulse' : 
								healthState === 'error' ? 'bg-rose-500' : 'bg-slate-400'
							}`} />
							<span className="text-sm font-bold uppercase tracking-wider">{healthMessage}</span>
						</div>
					</div>
				</div>
			</header>
			...
			<main className="grid grid-cols-1 lg:grid-cols-2 gap-8">
				...
				<section className="glass-panel overflow-hidden flex flex-col">
					<div className="p-6 border-b border-slate-100 bg-slate-50/50 flex items-center gap-3">
						...
						<div>
							...
							<p className="text-sm text-slate-500">Configure spatial and temporal parameters</p>
						</div>
					</div>

					<form onSubmit={(event) => void handleExtractSubmit(event)} className="p-6 space-y-6 flex-1">
						{/* Bounding Box */}
							...
							<div className="grid grid-cols-2 md:grid-cols-4 gap-4">
								{[
									...
									<div key={item.field} className="space-y-1">
										<span className="text-[10px] font-semibold text-slate-400 ml-1">{item.label}</span>
										<input
											type="number"
											step="any"
											value={form[item.field as keyof ExtractFormState] as string}
											onChange={(event) => updateFormField(item.field as keyof ExtractFormState, event.target.value)}
						...
						<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
							<div className="space-y-2">
								<label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Date Selection</label>
								<input
									type="date"
									value={form.date}
							...
							<label className="flex items-center gap-3 cursor-pointer group">
								<div className="relative">
									<input
										...
										onChange={(event) => updateFormField('runFetch', event.target.checked)}
										className="sr-only peer"
									/>
									<div className="w-10 h-5 bg-slate-200 rounded-full peer peer-checked:bg-emerald-500 transition-colors after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-4 after:w-4 after:transition-transform peer-checked:after:translate-x-5 shadow-sm" />
								</div>
								<span className="text-sm font-semibold text-slate-700 group-hover:text-slate-900 transition-colors">Run Fetch Phase (GEE Export)</span>
							</label>
							...
							<label className="flex items-center gap-3 cursor-pointer group">
								<div className="relative">
									<input
										...
										onChange={(event) => updateFormField('fetchOnly', event.target.checked)}
										className="sr-only peer"
									/>
									<div className="w-10 h-5 bg-slate-200 rounded-full peer peer-checked:bg-blue-500 transition-colors after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-4 after:w-4 after:transition-transform peer-checked:after:translate-x-5 shadow-sm" />
								</div>
								<span className="text-sm font-semibold text-slate-700 group-hover:text-slate-900 transition-colors">Fetch Only (Skip Local Processing)</span>
							</label>
						</div>
						...
						<div className="space-y-4">
							...
							>
								<Settings className={`w-4 h-4 transition-transform duration-300 ${showAdvanced ? 'rotate-90 text-emerald-600' : ''}`} />
						...
						<div className="pt-4">
							<button
								...
								className="w-full py-4 bg-gradient-to-r from-emerald-600 to-blue-600 hover:from-emerald-700 hover:to-blue-700 text-white font-bold rounded-2xl shadow-lg shadow-emerald-600/20 transition-all active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 text-lg"
							>
							...
							{extractMessage && (
								<div className={`mt-4 p-3 rounded-xl text-sm font-medium flex items-center gap-2 ${
									extractState === 'error' ? 'bg-rose-50 text-rose-700 border border-rose-100' : 'bg-emerald-50 text-emerald-700 border border-emerald-100'
								}`}>
									{extractState === 'error' ? <AlertCircle className="w-4 h-4 shrink-0" /> : <CheckCircle2 className="w-4 h-4 shrink-0" />}
									{extractMessage}
								</div>
							)}
						</div>
				...
				{/* Job Monitor */}
				<section className="glass-panel overflow-hidden flex flex-col max-h-[850px]">
					<div className="p-6 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between">
						<div className="flex items-center gap-3">
							...
							<div>
								<h2 className="text-xl font-bold text-slate-900">Job Monitor</h2>
						...
						>
							<RefreshCw className={`w-3 h-3 ${jobsState === 'loading' ? 'animate-spin' : ''}`} /> Refresh
						</button>
					</div>
					...
					<div className="flex-1 overflow-y-auto p-6 space-y-4 custom-scrollbar">
						...
						{jobs.length === 0 ? (
							<div className="h-full flex flex-col items-center justify-center text-center space-y-4 py-20 opacity-40">
								...
								<p className="text-slate-500 font-medium italic">No extraction jobs found.<br/>Submit a request to see it here.</p>
							</div>
						) : (
							jobs.map((job) => (
								<article
								...
								>
									<div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
										<div className="flex items-center gap-3">
											<div className={`p-1.5 rounded-lg border ${
												job.status === 'complete' ? 'bg-emerald-50 border-emerald-100 text-emerald-600' :
												job.status === 'failed' ? 'bg-rose-50 border-rose-100 text-rose-600' :
									...
									<div className="grid grid-cols-2 gap-y-3 gap-x-6 text-[11px]">
										<div className="space-y-0.5">
											<p className="text-slate-400 uppercase font-bold tracking-tighter">Location</p>
											<p className="text-slate-700 font-semibold truncate">{job.location ?? 'N/A'}</p>
										</div>
										<div className="space-y-0.5">
											<p className="text-slate-400 uppercase font-bold tracking-tighter">Target Date</p>
											<p className="text-slate-700 font-semibold">{job.date ?? 'N/A'}</p>
										</div>
										<div className="space-y-0.5">
											<p className="text-slate-400 uppercase font-bold tracking-tighter">Started At</p>
											<p className="text-slate-700 font-semibold">{prettyTime(job.started_at)}</p>
										</div>
										<div className="space-y-0.5">
											<p className="text-slate-400 uppercase font-bold tracking-tighter">Finished At</p>
											<p className="text-slate-700 font-semibold">{prettyTime(job.finished_at)}</p>
										</div>
									</div>
				...
				{/* Metadata Viewer */}
				<section className="glass-panel overflow-hidden flex flex-col max-h-[600px]">
					<div className="p-6 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between">
						<div className="flex items-center gap-3">
							...
							<div>
								...
								<p className="text-sm text-slate-500">Latest generated configuration</p>
							</div>
						</div>
						<button 
						...
						>
							<RefreshCw className={`w-3 h-3 ${metadataState === 'loading' ? 'animate-spin' : ''}`} /> Refresh
						</button>
					</div>
					...
					<div className="flex-1 overflow-hidden flex flex-col p-6 space-y-4">
						<div className="flex items-center gap-2 text-[10px] font-mono text-slate-400 bg-slate-50 px-3 py-1.5 rounded-lg border border-slate-100 overflow-x-auto whitespace-nowrap">
							<Database className="w-3 h-3" /> {metadataPath || 'No metadata loaded'}
						</div>
						...
						<div className="flex-1 bg-slate-900 rounded-2xl p-4 overflow-auto border border-slate-800 relative group">
							<div className="absolute top-4 right-4 text-[10px] font-bold text-slate-600 uppercase tracking-widest opacity-50 group-hover:opacity-100 transition-opacity">JSON Viewer</div>
							{metadataContent ? (
				...
				<section className="glass-panel overflow-hidden flex flex-col lg:col-span-2">
					<div className="p-6 border-b border-slate-100 bg-slate-50/50 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
						<div className="flex items-center gap-3">
							...
							<div>
								<h2 className="text-xl font-bold text-slate-900">Dataset Summary</h2>
								<p className="text-sm text-slate-500">Tabular inspection of extraction outputs</p>
							</div>
						</div>
						<div className="flex items-center gap-4">
							...
							<button 
								...
								className="shrink-0 text-xs font-bold text-slate-400 hover:text-amber-600 transition-colors uppercase tracking-widest flex items-center gap-2 border border-slate-200 px-4 py-2 rounded-xl bg-white hover:border-amber-200 shadow-sm transition-all"
							>
								<RefreshCw className={`w-3 h-3 ${summaryState === 'loading' ? 'animate-spin' : ''}`} /> Refresh Table
							</button>
						</div>
					</div>

					<div className="flex-1 overflow-auto min-h-[400px] custom-scrollbar">
						{summaryState === 'error' && (
							...
							<table className="w-full text-left border-collapse">
								<thead>
									<tr className="bg-slate-50 border-b border-slate-200 sticky top-0 z-20">
										{summaryColumns.map((column) => (
											<th key={column} className="px-6 py-4 text-[10px] font-bold text-slate-500 uppercase tracking-wider whitespace-nowrap">
												{column.replace(/_/g, ' ')}
											</th>
										))}
									</tr>
								...
								<tbody className="divide-y divide-slate-100">
									{summaryRows.slice(0, 100).map((row, index) => (
										<tr key={`${index}-${row[summaryColumns[0]]}`} className="hover:bg-slate-50/80 transition-colors"> 
											{summaryColumns.map((column) => (
												<td key={`${index}-${column}`} className="px-6 py-4 text-xs text-slate-600 whitespace-nowrap font-medium">
													{column === 'status' ? getStatusBadge(row[column] as string) : String(row[column] ?? '-')}
												</td>
											))}
										</tr>
									))}
			...
			<footer className="text-center py-10 space-y-2 opacity-50">
				<p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.3em]">Operational Environment v1.0.4</p>
				<p className="text-xs text-slate-400">© 2026 CV-Lab-EL Systems. All rights reserved.</p>
			</footer>
		</div>
	)
}
